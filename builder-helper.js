...
function normChar(c={}){
  ...
  return {
    ...
    sections:{
      ...
      other:s.other||'',
      negative:s.negative||c.negative||''
    },
    variants:Array.isArray(c.variants)?c.variants:[]
  };
}
async function buildPromptNegativeLast(){
  ...
  $$('[data-role="char"]:checked').forEach(input=>{
    ...
    if(negToggle&&negToggle.checked&&c.sections.negative)negatives.push(c.sections.negative);
  ...
  if(negatives.length)blocks.push(`Negative prompt: ${negatives.join(', ')}`);
  return blocks.join('\n\n');
}
...
function installBuilderOverride(){
  ...
  if(buildBtn)buildBtn.onclick=async()=>{
    out.value=await buildPromptNegativeLast();
    toast('組み立てた');
  };
  if(copyBtn)copyBtn.onclick=async()=>{
    const text=out.value||await buildPromptNegativeLast();
    await copyText(text);
  };
  if(saveBtn)saveBtn.onclick=async()=>{
    const prompt=out.value||await buildPromptNegativeLast();
